...
<? endif ?>
<form class="form pb-5" name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>" method="post" enctype="multipart/form-data">
	<div class="row g-4">
		<?= bitrix_sessid_post() ?>
		...
		<? if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])) : ?>
			...
			<? foreach ($arResult["PROPERTY_LIST"] as $propertyID) : ?>
				...
				?>
					<input class="form-control bg-light rounded-2 border border-secondary text-secondary p-3 h-100" type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" id="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" value="<?= $value ?>" placeholder="" required>
					<label for="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" class="form-label fs-16 text-secondary position-absolute top-50"><? if (intval($propertyID) > 0) : ?><?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ?><? else : ?><?= !empty($arParams["CUSTOM_TITLE_" . $propertyID]) ? $arParams["CUSTOM_TITLE_" . $propertyID] : GetMessage("IBLOCK_FIELD_" . $propertyID) ?><? endif ?></label>
					<?
